...
animals.glm.poisson%>%
  summary()
```
From the output of our model, we could see that outcome_typeFOSTER is not significant, think about the scatter plot of time_at_shelter against outcome_type, there exits a very large time_at_shelter value equal to 69 which maybe an outlier. Build a new datasets without this outliers.
...
From summary, we could see that all other variables are significant in our model. However, both the deviance 8654.6 and pearson statistic 11552.79 are larger than Chi-Square(1666)  1762.07 which indicate a poor fit.
