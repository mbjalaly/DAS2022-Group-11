...
animals.glm.poisson%>%
  summary()
```
From the output of our model, we could see that outcome_typeFOSTER is not significant, think about the scatter plot of time_at_shelter against outcome_type, there exits a very large time_aat_shelter value equal to 69 which maybe an outliers. Build a new datasets without this outliers.
...
animals.glm%>%
  summary()
```
>>>>>>> b37e7451f03f6ebb43bbb52c389b52a9353198d6
```{r,check goodness-of-fit}
#person statistic
sum(resid(animals.glm ,type="pearson")^2)
#Chi-Square(1667)
<<<<<<< HEAD
qchisq(0.95,1666,ncp=0)
```

From summary, we could see that all other variables are significant in our model. However, both the deviance 8654.6 and pearson statistic 11064.29 are larger than Chi-Square(1666)  1762.07 which indicate a poor fit.
=======
...
```
<<<<<<< HEAD
Based on F-test, all variables could be included.
=======
...
```
<<<<<<< HEAD
The deviance of Quasi-Poisson model is equal to poisson model.
=======
...
```
We can see the deviance of negative binomial model is much smaller than the poisson one, and as for AIC, both model are equal.
=======
```{r}
...
```
<<<<<<< HEAD
Based on AIC, the negative binomial model after dealing with zero-exceed may be the best model.
=======
